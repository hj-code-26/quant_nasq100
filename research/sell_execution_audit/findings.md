...
## 10. 실계좌 진입일 프로브 결과 (2026-09-16 17:37 KST, 사용자 승인 후 실행)

`python research/sell_execution_audit/entry_probe.py` — 읽기 전용 가드(주문 생성·취소·정정
POST·외부 호스트·운영 DB 차단 자가 점검 통과). 실행 전후 `trading_decisions.db`·
`autotrade.log`·`autotrade.py`·`toss.py` sha256 동일.

- 체결 이력: CLOSED **327건**(FILLED 282·CANCELED 43·REJECTED 2, 2024-07-20~2026-09-15,
  최신순), 페이지 전량 읽음. OPEN 0건. 거래일 달력 300일.
- **판정: PASS — 보유 7종목 전부 진입일 검증, 수량 일치.**

| 종목 | 보유=복원 수량 | 진입일 | 보유 거래일 | 만기까지 |
|---|---|---|---|---|
| KDP | 1 | 2026-09-03 | 8 | 12 |
| MSTR | 1 | 2026-09-04 | 8 | 12 |
| SMCI | 4 | 2026-09-04 | 8 | 12 |
| MU | 0.058791 | 2026-09-08 | 6 | 14 |
| TSLA | 0.344463 | 2026-09-09 | 6 | 14 |
| TEAM | 0.138645 | 2026-09-09 | 5 | 15 |
| MRNA | 0.408386 | 2026-09-15 | 2 | 18 |

**정정**: 같은 스냅샷을 CLOSED 첫 20건만으로 재분석하면(예전 코드 재현) **결과가 같다.**
현재 보유분의 진입이 전부 최근 20건 안에 있기 때문이다. 즉 20건 페이징 결함은 **지금
운영에 영향을 준 원인이 아니라 잠재 결함**이다 — 보유 기간 중 다른 주문(취소 포함)이
20건 넘게 쌓이면 그때 드러난다. §8 의 "ENTRY_UNVERIFIED 오탐의 원인이었을 가능성"은 철회한다.
시간외 주문 미조회(§8 ②)도 현재 계좌에서는 발생하지 않았다.

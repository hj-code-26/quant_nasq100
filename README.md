...
**청산은 Claude 에 맡기지 않는다.** 보유 종목이 평단 대비 `STOP_LOSS_PCT`(기본 25) % 넘게 빠지거나
20일 수익률이 음수로 꺾이면 `MOMENTUM_EXIT=1` 일 때 전량 매도 주문이 강제로 나간다 — Claude 가 hold 를
고집하거나 판단 호출 자체가 실패해도 포지션이 방치되지 않는다. 손절은 계좌 값만 쓰므로 Claude 없이도 동작.
매수를 20일 수익률 양수로만 제한한 것의 대칭이다 (`make test` 로 이 규칙들을 점검).

단 **모멘텀 청산은 갈아탈 후보가 있을 때만** 나간다. 계좌 단위 시뮬(`backtest.py --daily`)에서 청산의
우위는 판 자리에 바로 재진입한다는 가정에서만 나왔고, 후보가 없으면 회전이 아니라 저점 매도일 뿐이다.
손절선이 15 가 아니라 25 인 것도 같은 백테스트 결과다 — 15 는 두 구간 모두에서 수익을 깎았다.
